...
renamu %>% 
  map(~.x %>% 
        ...
        select(-starts_with("catmuni")) %>%
        {
        if("consejo_local" %in% names(.)) select(., -starts_with("consejo_local") & -ends_with("x")  | -starts_with("consejo_local") & -ends_with("y")) else .
        } %>%  
        {
        if("tiene_tupa" %in% names(.)) select(., -starts_with("tiene_tupa") & -ends_with("x") | -starts_with("tiene_tupa") & -ends_with("y")) else .
        }
      )
#ponerle el año
